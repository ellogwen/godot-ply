...
func startup():
	...
	_editor_selection.selection_changed.connect(self._on_selection_change)
	_plugin.toolbar.toolbar.selection_mode_changed.connect(self._on_selection_mode_change)
	_plugin.toolbar.toolbar.transform_mode_changed.connect(self._on_transform_mode_change)

func teardown():
	_editor_selection.selection_changed.disconnect(self._on_selection_change)
	_plugin.toolbar.toolbar.selection_mode_changed.disconnect(self._on_selection_mode_change)
	_plugin.toolbar.toolbar.transform_mode_changed.disconnect(self._on_transform_mode_change)
	show_spatial_gizmo()
...
func handle_click(camera, event):
	if event.pressed and is_selecting():
		...
		var root = _plugin.get_tree().get_edited_scene_root()
		var instances = RenderingServer.instances_cull_ray(ray_pos, ray, root.get_world().scenario)
		var target = null
		match mode:
			SelectionMode.FACE:
				target = Face
