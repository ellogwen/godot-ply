...
func _ready() -> void:
	...
	value_input.connect("text_entered", Callable(self, "_on_value_input_entered"))
	value_input.connect("focus_exited", Callable(self, "_on_value_input_focus_exited"))
	add_child(value_input, true)
...
func _draw() -> void:
	...
	draw_string(
		font, Vector2(round(sb.get_offset().x + string_width + sep), vofs), numstr, 0, -1, 16, fc, number_width
	)
